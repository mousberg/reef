  ...
  return (
    <div className={`space-y-3 ${className}`}>
      <h4 className="text-[#2F3037] font-medium text-sm flex items-center gap-2">
        <svg className="w-4 h-4 text-[#556B5D]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {toolCalls.map((toolCall, index) => (
        <div key={toolCall.id || index} className="border border-[rgba(55,50,47,0.12)] rounded-lg p-3 bg-white/50">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-[#2F3037] font-medium text-sm">{toolCall.function.name}</span>
