...
export function TraceTimeline({ traces, spans, onTraceSelect, onSpanSelect, className = "" }: TraceTimelineProps) {
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      <h3 className="text-[#2F3037] font-medium text-base flex items-center gap-2">
        <svg className="w-5 h-5 text-[#556B5D]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <span className={`text-xs font-medium capitalize px-2 py-1 rounded-full ${
                    ...
                    item.status === 'failed' ? 'text-red-700 bg-red-50' :
                    item.status === 'pending' ? 'text-yellow-700 bg-yellow-50' :
                    item.status === 'running' ? 'text-blue-700 bg-blue-50' :
                    'text-[#556B5D] bg-[#F7F5F3]'
                  }`}>
                    {item.status}
                  </span>

                  {item.data && 'metadata' in item.data && item.data.metadata && (
                    <span className="text-[#556B5D] text-xs">
                      {item.type === 'trace' ?
