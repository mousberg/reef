...
export function ToolNode({ data }: ToolNodeProps) {
  ...
  const getLucideIcon = (toolName: string) => {
    ...
    if (name.includes('twitter') || name.includes('x.com') || name.includes(' x ')) return Twitter;
    ...
    if (name.includes('freshworks') || name.includes('freshdesk')) return Users2;
    ...
    if (name.includes('postgres') || name.includes('postgresql')) return Database;
