    ...
    <div className="space-y-3">
      ...
      {isExpanded && (
        <div className="space-y-3 pt-2 border-t border-green-200 dark:border-green-800">
          <div>
            <h4 className="font-semibold text-green-700 dark:text-green-300 mb-1">Main Task</h4>
            <p className="text-sm text-green-800 dark:text-green-200">{workflowState.main_task}</p>
          </div>

          <div>
            <h4 className="font-semibold text-green-700 dark:text-green-300 mb-1">Agents ({workflowState.agents?.length || 0})</h4>
            <div className="space-y-2">
              {workflowState.agents?.map((agent, index) => (
                <div key={index} className="border-l-2 border-green-300 pl-2">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-medium text-green-700 dark:text-green-300">{agent.name}</span>
                    <span className="text-xs bg-green-100 dark:bg-green-900/50 px-2 py-0.5 rounded">
                      {agent.tools?.length || 0} tools
                    </span>
                  </div>
                  <p className="text-xs text-green-600 dark:text-green-400">{agent.description}</p>
                </div>
              )) || <p className="text-xs text-green-600 dark:text-green-400">No agents defined yet</p>}
            </div>
          </div>

          {workflowState.relations && (
            <div>
              <h4 className="font-semibold text-green-700 dark:text-green-300 mb-1">Agent Relations</h4>
              <p className="text-sm text-green-800 dark:text-green-200">{workflowState.relations}</p>
            </div>
          )}
        </div>
      )}
