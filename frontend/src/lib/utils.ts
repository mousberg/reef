...
export const cls = (...c: (string | boolean | undefined | null)[]) => c.filter(Boolean).join(" ")
 ...
 */
export function useLocalStorage<T>(key: string, defaultValue: T, isJSON: boolean | null = null): [T, (newValue: T | ((prevValue: T) => T)) => void] {
  ...
  const setStoredValue = useCallback((newValue: T | ((prevValue: T) => T)) => {
    try {
      ...
      if (typeof window !== "undefined") {
        const itemToStore = shouldUseJSON ? JSON.stringify(valueToStore) : String(valueToStore);
